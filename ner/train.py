...
from ner.early_stopping import EarlyStopping
from ner.label_smoothing import LabelSmoothingCrossEntropy
    ...
    for local_step, (x,y) in enumerate(epoch_iterator):
        ...
        if (local_step + 1) % opt.gradient_accumulation_steps == 0:
            ...
            if opt.eval_and_save_steps > 0 and global_step != 0 and global_step % opt.eval_and_save_steps == 0:
                ...
                if local_best_eval_f1 < eval_f1:
                    local_best_eval_f1 = eval_f1
                if writer:
...
def set_path(config):
    ...
    opt.chunk_path = os.path.join(opt.data_dir, opt.chunk_filename)


def prepare_datasets(config, train_path=None, valid_path=None):
    opt = config['opt']
...
def reduce_bert_model(config, bert_model, bert_config):
    ...
    if remove_layers is not "":
        ...
        for layer_idx in layer_indexes:
            if layer_idx < 0 or layer_idx >= bert_config.num_hidden_layers:
                continue
...
def train(opt):
    ...
    set_detect_anomaly:
        ...
        Running the forward pass with detection enabled will allow the backward pass to print the traceback 
        of the forward operation that created the failing backward function.
    ...
    with temp_seed(opt.seed):
        ...
        for epoch_i in range(opt.epoch):
            ...
            eval_loss, eval_f1, best_eval_f1 = train_epoch(model, config, train_loader,
                                                           valid_loader, epoch_i, best_eval_f1)
            # early stopping
...
def main():
    ...
    parser.add_argument('--chunk_filename', type=str, default='chunk.txt')
    ...
    parser.add_argument('--criterion', type=str, default='CrossEntropyLoss',
                        help="training objective, 'CrossEntropyLoss' | 'LabelSmoothingCrossEntropy', "
    ...
    parser.add_argument('--bert_remove_layers', type=str, default='',
                        help="Specify layer numbers to remove during finetuning e.g. 8,9,10,11 "
                             "to remove last 4 layers from BERT base(12 layers)")
